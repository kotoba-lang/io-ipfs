 ...
 */
export async function pinBlob(apiUrl, blob) {
    let body;
    ...
    const res = await fetch(`${apiUrl.replace(/\/+$/, "")}/api/v0/add?pin=true&cid-version=1`, { method: "POST", body: form });
    if (!res.ok) {
        throw new Error(`[etzhayyim-sdk/ipfs] pin failed: ${res.status} ${await res.text()}`);
    }
 ...
 */
export async function fetchBlob(gatewayUrl, cid) {
    const res = await fetch(`${gatewayUrl.replace(/\/+$/, "")}/ipfs/${encodeURIComponent(cid)}`);
    if (!res.ok) {
        throw new Error(`[etzhayyim-sdk/ipfs] fetch failed: ${res.status} ${await res.text()}`);
    }
 ...
 */
export async function nodeInfo(apiUrl) {
    ...
    return { id: idData.ID, version: versionData.Version };
}
